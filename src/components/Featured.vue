<template>
  <v-row>
    <v-col v-for="(feature, i) in featured" :key="i" cols="3" sm="3">
      <v-card class="mx-auto my-12 pb-4" max-width="374">
        <v-img height="250" class="mx-4" :src="feature.img"></v-img>
        <v-card-item class="mt-n4">
          <v-card-title class="text-center" :title="feature.title">{{ feature.title }}</v-card-title>
        </v-card-item>
        <v-card-text>
...
const featured = ref<Array<Feature>>([])
